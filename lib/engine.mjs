// LOOPER — the engine. One call, the whole operating loop:
//
...
import { buildBacklogViews } from "./backlog.mjs";
import { buildCapacityByPlanningGroup } from "./capacity-view.mjs";
import { normalizeBatch, buildHierarchyIndex } from "./hierarchy.mjs";
...
export function prioritize(initiatives, opts = {}) {
  ...
  const normalized = normalizeBatch(initiatives);
  const hierarchy = buildHierarchyIndex(normalized);

  // 1) Intake — structural + evidence gate (CADMUS + Appendix A).
  const { accepted, rejected } = intake(normalized, ledger);
  ...
  // 7) Portfolio views — backlog cuts + capacity by Planning Group.
  const backlog = buildBacklogViews(tiered);
  const capacityView = buildCapacityByPlanningGroup(tiered, { capacity });
  ...
  return {
    ...
    capacityView,
    hierarchy: {
      roots: tiered.filter((it) => !it.parentId).length,
      withParent: tiered.filter((it) => it.parentId).length,
      index: {
        childCounts: Object.fromEntries(
          [...hierarchy.childrenOf.entries()].map(([id, kids]) => [id, kids.length])
        ),
      },
    },
    stats: {
